...
class GithubClient:
    ...
    def get_events(self) -> Optional[List[Dict[str, Any]]]:
        """
        Fetch public events from the GitHub REST API using ETag for efficient polling.
        Returns a list of event dicts, None if no new events, or None on failure.
        """
        url = f"{self.http_rest_api_url}/events"
        for attempt, token in self._attempts():
            logger.info(f"GET {url} using {attempt}...")
            headers = self._headers(token)
            if self.last_etag:
            ...
            try:
                ...
                if etag_header:
                    self.last_etag = etag_header
                if response.status_code == http.HTTPStatus.NOT_MODIFIED:
                    logger.info("No new events (304 Not Modified). Returning None.")
                    return None
                if response.status_code == http.HTTPStatus.OK:
                    return response.json()
                logger.warning(f"Failed with {attempt}: HTTP {response.status_code} → trying next")
                ...
                logger.warning(f"GET request failed using {attempt}: {e}")
        logger.warning("All attempts failed for GET /events request.")
        return None
